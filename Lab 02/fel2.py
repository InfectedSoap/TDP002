...
def parse_symbol(string, start):
    ...
    if result == '':					#if string number is false and does not get coverted to float.
   		number = False
    if number:
    	return float(result), end			#ValueError: could not convert string to float
    else:
        return result, end
